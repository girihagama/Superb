...
public class adminReview extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int item = Integer.parseInt(request.getParameter("itemNumber"));
            String admin = request.getParameter("admin");

            User_Class usr = new User_Class();

            if (usr.userValidate(admin) && usr.requestAccountType(admin).equalsIgnoreCase("admin")) {
                out.print("Admin");
                session.setAttribute("adminName", admin);
                session.setAttribute("itemNumber", item);

                response.sendRedirect("ViewAd?itemNumber=" + item);
            } else {
                session.setAttribute("adminName", null);
                session.setAttribute("itemNumber", null);

                out.print("<center>Non-admin</center>"
                        + "<script>"

    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        while (details.hasNext()) {
            ...
            if (item.getName() == null) {
                if (item.getFieldName().equalsIgnoreCase("itemNumber")) {
                    itemNumber = Integer.parseInt(item.getString());
                } else if (item.getFieldName().equalsIgnoreCase("adType")) {
        ...
        if (!auth.checkEditAuthentication(username, itemNumber)) {
            response.setContentType("text/html;charset=UTF-8");
        ...
        try {
            ...
            if (newItem.updateItem(itemNumber, username, categoryMain, categorySub, locationDistrict, locationCity, adType)) {

                //getting generated key for new item
                out.println("Item " + this.itemNumber + " updated!");

                //adding item info
                info.updateItem(itemNumber, title, description, price, negotiable, voice);
                out.println("Information of item " + this.itemNumber + " is updated!");
            }
        } catch (Exception e) {
        ...
        //this is used to fix '0.jpg to cover.jpg' bug!
        String root = getServletContext().getRealPath("/");
        recorrectCoverImage(root + destinationPath + itemNumber);

        //redirecting
        HttpSession session = request.getSession();
        session.setAttribute("submission", "true");
        response.sendRedirect("ViewAd?itemNumber=" + itemNumber);
    }
...
//            Logger.getLogger(modifyAd.class.getName()).log(Level.SEVERE, null, ex);
//        }
    }
...
//            }
            if (!path.isDirectory() || !path.exists()) {
                path.mkdirs();
                this.imageNumber = path.listFiles().length;
            } else if (path.exists()) {
                // update image number
                this.imageNumber = path.listFiles().length;
            }
    ...
    private void recorrectCoverImage(String file) throws IOException {
        File pathUnfix = new File(file + File.separator + "0.jpg");
        pathUnfix.setExecutable(true, true);
        pathUnfix.setReadable(true, true);
        pathUnfix.setWritable(true, true);
        
        File pathFixed = new File(file + File.separator + "cover.jpg");
        pathFixed.setExecutable(true, true);
        pathFixed.setReadable(true, true);
        pathFixed.setWritable(true, true);
        
        if(pathUnfix.exists()){
            Files.move(pathUnfix.toPath(), pathFixed.toPath());
        }
    }
}
